...
<div id="{$formAtts.form_id|escape:'html':'UTF-8'}" class="block carousel slide full-slider {(isset($formAtts.class)) ? $formAtts.class : ''|escape:'html':'UTF-8'}"
	...
	<div role="listbox" class="carousel-inner">
		...
		<div class="carousel-item {if $count == 0}active{/if}">
			{if $slide['img']}
			<img {if $aplazyload}class="lazy" data-src{else}src{/if}="{$slide['img']}{*full link can not validate*}" alt="{$slide['title'] nofilter}{* HTML form , no escape necessary *}" loading="lazy"/>
			{else}
				{if isset($slide['image_link']) && $slide['image_link']}
				<img {if $aplazyload}class="lazy" data-src{else}src{/if}="{$slide['image_link']}{*full link can not validate*}" alt="{$slide['title'] nofilter}{* HTML form , no escape necessary *}" loading="lazy"/>
				{/if}
			{/if}
