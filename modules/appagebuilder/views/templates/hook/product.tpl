					...
					{foreach from=$images item=image name=thumbnails}
						...
						<li id="thumbnail_{$image.id_image|intval}">
							<a href="{$link->getImageLink($product.link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}" data-idproduct="{$product.id_product|intval}" rel="other-views" class="thickbox-ajax-{$product.id_product|intval}{if $smarty.foreach.thumbnails.first} shown{/if}" title="{$image.legend|htmlspecialchars}">
								<img id="thumb_{$image.id_image|intval}" {if $aplazyload}data-src{else}src{/if}="{$link->getImageLink($product.link_rewrite, $imageIds, 'cart_default')|escape:'html':'UTF-8'}" alt="{$image.legend|htmlspecialchars}" rel="{$link->getImageLink($product.link_rewrite, $imageIds, 'home_default')}" class="leo-hover-image{if $aplazyload} lazy{/if}" loading="lazy"/>
							</a>
						</li>
					{/foreach}
				{/if}
