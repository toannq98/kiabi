    					...
    					<div class="manufacturer-image-container image">
    						<a title="{l s='%s' sprintf=[$manu.name] mod='appagebuilder'}" href="{$link->getmanufacturerLink($manu.id_manufacturer, $manu.link_rewrite)|escape:'html':'UTF-8'}" itemprop="url">
                                                    {if isset($formAtts) && isset($formAtts.lazyload) && $formAtts.lazyload}
                                                        ...
                                                        <img class="img-fluid lazyOwl" src="" data-src="{$img_manu_dir}{*full link can not escape*}{$manu.id_manufacturer|intval}-{$image_type|escape:'html':'UTF-8'}.jpg" alt="{$manu.name|escape:'html':'UTF-8'}" itemprop="image" loading="lazy"/>
                                                    {else}
                                                        <img class="img-fluid{if $aplazyload} lazy{/if}" {if $aplazyload}data-src{else}src{/if}="{$img_manu_dir}{*full link can not escape*}{$manu.id_manufacturer|intval}-{$image_type|escape:'html':'UTF-8'}.jpg" alt="{$manu.name|escape:'html':'UTF-8'}" itemprop="image" loading="lazy"/>
                                                    {/if}
    						</a>
    					</div>
    				</div>
