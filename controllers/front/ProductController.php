...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    protected function assignAttributesGroups($product_for_template = null)
    {
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            /* Use product photos as color photos */
            $sql = 'SELECT a.*, b.id_image FROM `'._DB_PREFIX_.'product_attribute_combination` as a ';
            $sql .= ' LEFT JOIN '._DB_PREFIX_.'product_attribute_image as b ON (a.id_product_attribute = b.id_product_attribute) ';
            $sql .= ' WHERE a.id_product_attribute IN (SELECT id_product_attribute FROM `'._DB_PREFIX_.'product_attribute` where id_product = '.$this->product->id.')';
            ...
            if(count($id_attributes)){
                $sql.= ' AND a.id_attribute IN ('.implode(',', $id_attributes).')';
            }
            $sql.= ' AND b.id_image <> "" GROUP BY id_attribute';
            $custon_attributes = Db::getInstance()->executeS($sql);
            $img_url = $this->context->link->getImageLink($this->product->name, 25, 'small_default');
            if(isset($groups[2])){
                foreach ($groups[2]['attributes'] as $key => &$color) {
                    foreach ($custon_attributes as $key_2 => $value_2) {
                        if($value_2['id_attribute'] == $key){
                            $color['image'] = $this->context->link->getImageLink($this->product->name, $value_2['id_image'], 'small_default');
                        }
                    }
                }
            }
